    ...
    while access != 1:
        ...
        for (x, y, w, h) in faces:
            ...
            if conf >= 80 and count_time <= 50:
                ...
                engine.say('Доступ разрешён, начинаем приём')
                exit()
            else:
                make_photo_to_db()
                engine.say('Успешная регистрация, начинаем приём')
                exit()
...
def make_photo_to_db():
    ...
    for i in range(5):
        ret, img = cap.read()
        cv2.imwrite(f"Face_Images/{labels[-1]}{i}.png", img)
        time.sleep(2)
    cap.release()
